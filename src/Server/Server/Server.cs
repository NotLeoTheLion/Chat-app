            ...
            if (!exit)
            {
                startButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg("Serveris paleistas"));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg("Serveris sustabdytas"));
                    }
                });
            }
        }
        ...
        private void AddToGrid(long id, string name)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    totalLabel.Text = string.Format("Prisijunge klientai: {0}", clientsDataGridView.Rows.Count);
                });
            }
        }
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive)
            {
                ...
                else
                {
                    ...
                    {
                        error = true;
                        Log(SystemMsg("Adresas negalioja"));
                    }
                }
                int port = -1;
                if (number.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Port butinas"));
                }
                else if (!int.TryParse(number, out port))
                {
                    error = true;
                    Log(SystemMsg("Port negalioja"));
                }
                else if (port < 0 || port > 65535)
                {
                    error = true;
                    Log(SystemMsg("Port skaicius uz ribu"));
                }
                if (username.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Username butinas"));
                }
                if (!error)
                {
